...
def sync_file_num(src_files: List[str], dest_files: List[str]):
...
def sync_file_context(src_files: List[str], dest_files: List[str]):
    dest_files = os.listdir(DEST)
    dest_files = list(filter(lambda dest_file: dest_file.endswith('.ps1'), dest_files))
...
def backup(src_files: List[str], backup_path: str):
