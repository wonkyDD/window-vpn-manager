...
def timer(elapsed_time: float):
    # min, sec로 변환
    if elapsed_time >= 60.0:
        min = int(elapsed_time//60)
        sec = elapsed_time - (min*60.0)
        print(f'Taken time : {min}min {round(sec, 1)}sec')
    ...
    try:
        ...
        for genre, target_link in TARGET_LINKS.items():
            ...
            while True:
                ...
                titles_and_links: List[Tuple] = yd_manager.get_all_videolinks()
                
                videoids = []
                for title_and_link in titles_and_links:
                    title, link = title_and_link
                    
                    videoid: int = yd_manager.parse_videoid(link)
