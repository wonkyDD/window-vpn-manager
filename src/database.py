...
cur.executemany('INSERT INTO test (name) 
                VALUES(:name)', [{'name': 'james'}, {'name': 'amy'}]) << 통과!
...
from .settings import BACKUP_SQL_NAME, BACKUP_SQL_PATH, DB_NAME, DB_PATH, DB_TABLE_NAMES
...
def backup_database():
    ...
    try:
        ...
        with db:
            with open(
                f"{BACKUP_SQL_PATH}{BACKUP_SQL_NAME}", "w", encoding="utf-8"
            ) as p:
